...
cmd_get_intra_coords="awk '{FS=OFS=\"\\t\"} {print \$1, \$2, gensub(/.*[^A-Za-z]END=([0-9]+);.*/, \"\\\\1\", \"g\"), \$NF}'"
...
cmd_get_end2_coords="awk '{FS=OFS=\"\\t\"} {chr=gensub(/.*[[:blank:]][A-Za-z]*[\[\]]([chrNCGLhs37d_]*[0-9MTXY.]+):[0-9]+[\[\]].*/, \"\\\\1\", \"g\"); pos1=gensub(/.*[[:blank:]][A-Za-z]*[\[\]][chrNCGLhs37d_]*[0-9MTXY.]+:([0-9]+)[\[\]].*/, \"\\\\1\", \"g\")} (pos1!=0){print chr,pos1-1,pos1,\$NF} (pos1==0){print chr,pos1,pos1+1,\$NF}'"
