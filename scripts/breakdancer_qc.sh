...
if [ "$num1" -lt "$num2" ]; then
	...
	echo "PASS: $my_config contains at least one line per read group in $my_bam." >> $out
fi
...
# num1=$(samtools view -H $t_bam | grep -c @RG)
# num2=$(grep -c $t_bam $my_config)
# if [ "$num1" -lt "$num2" ]; then
# 	echo "FAIL: Tumor BAM may be missing RG or LB information in header." > $out
# else
# 	echo "PASS: config file contains at least one line per read group in tumor BAM file." > $out
# fi

# num1=$(samtools view -H $n_bam | grep -c @RG)
# if [ "$num1" -lt "$num2" ]; then
# 	echo "FAIL: Normal BAM may be missing RG or LB information in header." >> $out
# else
# 	echo "PASS: config file contains at least one line per read group in normal BAM file." >> $out
# fi
